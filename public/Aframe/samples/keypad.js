AFRAME.registerComponent('my_keyboard_controller', {
    init: function(){
      ...
      this.el.addEventListener('superkeyboardinput', function(event){
        ...
        if (event.detail.value === "1234") { 
            ...
            numpad.setAttribute("super-keyboard", "label:SUCCESS; labelColor: green");
            console.log(numpad);
        } else {
          var numpad = document.querySelector('#keyboard-display');
          numpad.setAttribute("super-keyboard", "label:ERROR; labelColor: red");
          console.log(numpad);
          removeError();
        }
  ...
  async function removeError(){
    setTimeout(function() {
      var numpad = document.querySelector('#keyboard-display');
      numpad.setAttribute("super-keyboard", "label:Enter Password; labelColor: black");
    }, (1.5 * 1000));
  }
